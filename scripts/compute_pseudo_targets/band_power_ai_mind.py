...
def _single_ocular_state(config):
    ...
    for feature in band_powers.columns:
        ...
        band_powers[["Dataset", "Subject-ID", feature]].to_csv((folder / f"{feature_name}_ai_mind").with_suffix(".csv"),
                                                               index=False)
...
def main():
    ...
    # Read config file
    # ---------------
    # todo: unnecessary with a new script...
    with open(os.path.join(os.path.dirname(__file__), "config_files", "band_power_ai_mind.yml")) as file:
