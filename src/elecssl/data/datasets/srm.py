...
class SRM(EEGDatasetBase):
    ...
    >>> SRM().load_targets(subject_ids=("sub-007", "sub-002", "sub-003"), target="age")  # doctest: +SKIP
    array([37, 29, 62])
    >>> SRM().load_targets(subject_ids=("sub-007", "sub-002", "sub-003"), target="log_age")  # doctest: +ELLIPSIS +SKIP
    array([3.6..., 3.3..., 4.1...])
    >>> len(SRM().get_subject_ids())  # doctest: +SKIP
    111
    >>> len(SRM().channel_name_to_index())  # doctest: +SKIP
    64
    >>> SRM().channel_name_to_index()  # doctest: +NORMALIZE_WHITESPACE, +SKIP
    ...
    ... # doctest: +SKIP
    {<class 'int'>}
    >>> SRM().channel_system.montage_name  # doctest: +SKIP
    'standard_1020'
    """
