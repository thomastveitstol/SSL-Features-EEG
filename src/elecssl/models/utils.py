...
def _yaml_optuna_not_a_hyperparameter_dict(loader, node):
    """This is supposed to be used if it is convenient to loop through HPs (or store something in the HPD config file),
    but you don't actually want it to be registered as an HP by the trial object"""
    return "not_a_hyperparameter", loader.construct_mapping(node)
...
def add_yaml_constructors(loader: Type[yaml.SafeLoader]) -> Type[yaml.SafeLoader]:
    ...
    loader.add_constructor("!NotAHyperparameterDict", _yaml_optuna_not_a_hyperparameter_dict)
    return loader
